...
class HTTPRoutine extends _Tools.EventManager {
  ...
  async connect(host, port, callback = () => {}, error = () => {}) {
    ...
    try {
      ...
      this.log("<-", await (0, _tools.call)(`${this.host}:${this.port}`, "#D11")); // set FocusMode to manual
      this.log("<-", await (0, _tools.call)(`${this.host}:${this.port}`, "#AXIFFF")); // set IrisMode to open
