...
class OSCReceiverHelper {
  constructor(port, ip = '0.0.0.0', readyCallback = () => {}) {
    this.oscServer = new _nodeOsc.Server(port, ip, readyCallback);
  }
...
class Gamepad {
  ...
  constructor() {
    ...
    this.input.onMessage('/mouse/position', message => {
      this.handlers.joystick[Gamepad.JOYSTIC_DESC.RIGHT] && this.handlers.joystick[Gamepad.JOYSTIC_DESC.RIGHT].forEach(handler => {
        handler({
  ...
  onJoystick(description, callback) {
    if (!Object.values(Gamepad.JOYSTIC_DESC).includes(description)) throw new Error(`onJoystick wait for Gamepad.JOYSTIC_DESC as first parameter. You give "${description}".`);
  ...
  onButton() {
    if (!Object.values(Gamepad.BUTTON_DESC).includes(description)) throw new Error(`onButton wait for Gamepad.BUTTON_DESC as first parameter. You give "${description}".`);
    if (typeof callback !== 'function') throw new Error(`onButton wait for function as second parameter. You give "${typeof callback}".`);
