...
class RequestHelper{
	...
	async sender(control, withParams = true){
		...
		return {res, counter : this.counter}
	}
...
export default class PTZController extends RequestHelper {
	constructor(ip = '192.168.0.10', port = 80){
		super({
				host: ip,
				port: port,
	...
	async setPanTiltSpeed(pan, tilt){
		...
		if( !Controls.PanTiltSpeed.params.pan.changed && 
			!Controls.PanTiltSpeed.params.tilt.changed
		){
			return false;
		}
	...
	async getPanTiltZoomFocusIris(){
		...
		// const result = ;//await this.sender(Controls.GetPanTiltZoomFocusIris);
		// if(!result.startsWith("pTV"))
		// 	throw new Error(`Something went wrong with GetPanTiltZoomFocusIris : ${result}`);
