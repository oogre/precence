...
var curveShape = script.addTargetParameter("CurveShape","Description of my target param"); 			//This will add a target parameter (to reference another parameter)
...
function setValue(values, multiplexIndex) {
	...
	//script.log("bonjour");
	curveShape.getParent().currentTime = values[0];
	
	//root.sequences.getChild("Curve").play();
	//root.sequences.getChild("Curve").pause();

	script.log(curveShape);
